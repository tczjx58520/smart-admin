<template>
  <Modal v-model="mymoadlStat"
    ...
    <div slot="header"
      ...
      <span>{{ $t('usermanage_view.editUser') }}</span>
    </div>
    <div>
      <Card dis-hover>
        ...
        <Form ref="form"
              ...
              :label-width="100"
              :rules="ruleValidate">
          <FormItem :label="$t('usermanage_view.firstname')"
                    prop="firstname">
          ...
          <FormItem :label="$t('usermanage_view.lastname')"
                    prop="lastname">
            <Input v-model="addformbase.lastname"></Input>
          </FormItem>
          <FormItem :label="$t('usermanage_view.phone')"
                    prop="phone">
            <Input v-model="addformbase.phone"></Input>
          </FormItem>
          <FormItem :label="$t('usermanage_view.mail')"
                    prop="mail">
            <Input v-model="addformbase.mail"></Input>
          </FormItem>
          <FormItem :label="$t('usermanage_view.post')"
                    prop="postOa">
            <!-- <Input v-model="addformbase.postOa"></Input> -->
            <Select v-model="addformbase.postOa"
                    style="width:100%">
              <Option v-for="item in postList"
                      :value="item.id"
                      v-bind:key="item.id">{{ item.postName }}</Option>
            </Select>
          </FormItem>
          <FormItem :label="$t('usermanage_view.organization')"
                    prop="organizationOa">
            <Input v-model="addformbase.organizationOaName"
                   readonly
            ...
            <div class="department-wrap"
                 v-if="isShowTree">
              <DepartmentEmployeeTree :isDepartment="true"
                                      @on-select="selectDepartmentOrEmployee"
                                      ref="departmentEmployeeTree"></DepartmentEmployeeTree>
            </div>
          </FormItem>
          <FormItem :label="$t('usermanage_view.belongRole')"
                    prop="roleOa">
            <!-- <Input v-model="addformbase.roleOa"></Input> -->
            <Select v-model="addformbase.roleOa"
                    style="width:100%">
              <Option v-for="item in roleNames"
                      :value="item.id"
                      v-bind:key="item.id">{{ item.roleName }}</Option>
            </Select>
          </FormItem>
          <FormItem :label="$t('usermanage_view.workstat')"
                    prop="stat">
            <Select v-model="addformbase.stat"
                    style="width:100%">
        ...
        <Divider />
        <Form ref="form2"
              :model="addformaccount"
              label-position="right"
              :label-width="100"
              :rules="ruleValidate2">
          <FormItem :label="$t('usermanage_view.logininfo')"
                    prop="account">
            <Input v-model="addformaccount.account"></Input>
          </FormItem>
          <FormItem :label="$t('usermanage_view.loginpsd')"
                    prop="passwd">
...
export default {
  ...
  data () {
    ...
    const checkphone = (rule, value, callback) => {
      console.log('phone===>', this.addformbase.phone);
      if (!this.addformbase.phone) {
        return callback(new Error('please enter phone'));
      }
      setTimeout(() => {
        console.log(String(this.addformbase.phone).length);
  ...
  watch: {
    async modalstat () {
      ...
      this.addformbase.id = this.editinfo.id;
      ...
      this.addformbase.organizationOaName = this.editinfo.organizationOaName;
      this.addformbase.phone = this.editinfo.phone;
      this.addformbase.mail = this.editinfo.email;
      this.addformbase.roleOa = String(this.editinfo.roleOa);
  ...
  methods: {
    ...
    async getBaseRole () {
      ...
      // });
      // console.log('needRolesId=========', needRolesId);
      ...
      roleApi.getAllRole2().then(res => {
        this.roleNames = [];
        this.roleNames = res.data.content;
        console.log(33333333, this.roleNames);
      });
    },
    cancel () {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.$refs['form2'].validate((valid) => {
            if (valid) {
              usermanagelApi.updateUser(this.addformbase, this.addformaccount).then(res => {
                if (res.ret === 200) {
                  this.$emit('updateStat', false);
