            ...
            <Card dis-hover>
              <div style="display:flex">
                ...
                <div style="width: 80%">
                  ...
                  <Table highlight-row border ref="selection" :columns="columns4" :data="emplist"  max-height="400" :loading="emp_loading" @on-current-change="currentRow"></Table>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      columns4: [
        ...
        {
          title: this.$t('usermanage_view.book'),
          key: 'address',
          width: '100'
        },
        {
  ...
  methods: {
    ...
    handsave () {
      ...
      if (this.param.id !== null && this.param.id !== '' && this.param.id !== undefined) {
        this.$Message.success(this.$t('addSuccess'));
        this.$emit('updateStat', false);
        this.modal_loading = false;
        this.$emit('getEmpId', this.param);
      } else {
    ...
    currentRow (currentRow) {
      console.log('currentRow.personName', currentRow.personName);
      console.log('currentRow.id', currentRow.id);
      this.param.personName = currentRow.personName;
      this.param.id = currentRow.id;
    },
    // 保存分页选中
    selectService (selection) {
      ...
      console.log(this.formValidate.serviceIdList);
    },
    // 根据id选中
    selectFromId (showList, selectList) {
      let that = this;
      if (selectList) {
        console.log('执行===》', showList, selectList);
        for (const i in showList) {
          console.log(showList[i], selectList.includes(showList[i].id));
