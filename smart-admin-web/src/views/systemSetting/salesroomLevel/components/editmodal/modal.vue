  ...
  watch: {
    modalstat () {
      ...
      if (this.mymoadlStat) {
        ...
        SalesRoomlevel.getSalesRoomlevelDetail(this.editinfo).then(res => {
          console.log('res========', res.data.content[0]);
          this.addformbase.levelName = res.data.content[0].levelName;
          this.addformbase.employeeStandards = res.data.content[0].employeeStandards;
          this.addformbase.stat = 1;
          this.addformbase.id = res.data.content[0].id;
          this.data_leader = res.data.content[0].commisonLeaderFinishVos;
          this.leaderform = res.data.content[0].commisonLeaderRule;
          this.managerform = res.data.content[0].commisonManagerRewardVo;
          this.data_manager = res.data.content[0].commisonManagerRules;
          this.data_oversale = res.data.content[0].commisonTeamOvers;
          // this.rankList = res.data.content[0].commisonTeamRanks;
          const temp = res.data.content[0].commisonTeamRanks;
          temp.map(item => {
            item.flagid = this.generateUUID();
            item.label = `第${item.rank}名奖金${item.reward}`;
            item.selected = false;
          });
          this.rankList = temp;
          this.data_team = res.data.content[0].commisonTeamRules;
        });
      }
    }
  },
  methods: {
    ...
    handsave () {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.isedit) {
            SalesRoomlevel.updateSalesRoomlevel(temp).then(res => {
              if (res.ret === 200) {
                this.$emit('updateStat', false);
                this.addformbase = Object.assign({}, defaultForm);
