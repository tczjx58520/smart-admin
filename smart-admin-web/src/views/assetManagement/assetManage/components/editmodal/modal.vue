      ...
      <Card dis-hover>
        ...
        >
          <Row>
            ...
            <Col span="12" v-if="showMap['年折旧资金']">
...
export default {
  ...
  data () {
    ...
    const validatePass4 = (rule, value, callback) => {
      if (
        this.addformbase.depreciationRate === '' &&
        this.addformbase.depreciationRate === null &&
        this.addformbase.depreciationRate === undefined
      ) {
        callback(new Error('Please enter'));
      } else {
        callback();
  ...
  watch: {
    modalstat () {
      ...
      if (this.mymoadlStat) {
        ...
        if (this.addformbase.classifyParentId) {
          this.addformbase.parentClassifyId = this.addformbase.classifyParentId.split(',').map(Number);
        }
        this.addformbase.custodiansId = this.addformbase.manageEmp;
        this.addformbase.manageEmp = this.addformbase.custodiansId;
        this.addformbase.manageEmpName = this.addformbase.custodiansName;
        this.addformbase.manageOrganize = this.addformbase.organizationId;
        this.addformbase.manageOrganizeName = this.addformbase.organizationName;
        this.addformbase.purchaseTimeStr = this.addformbase.purchaseTime;
        this.addformbase.registrationTimeStr = this.addformbase.registrationTime;
      }
    }
  },
  methods: {
    ...
    handsave () {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          assetManage.updatestorage(this.addformbase).then(res => {
            if (res.ret === 200) {
              this.$emit('updateStat', false);
    ...
    getShowList () {
      ...
      //   '资产编号': 'assetNum'
      // };
      fixedField.getEditShow().then(res => {
        console.log('res======', res);
