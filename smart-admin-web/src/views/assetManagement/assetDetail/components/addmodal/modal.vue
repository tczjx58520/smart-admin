      ...
      <Card dis-hover>
        ...
        >
          <div
            ...
            "
          ></div>
          <div>{{ filterSecondTitle(flag) }}</div>
        </div>
        <Divider />
        <Form
          ref="form"
        ...
        >
          <Row>
            ...
            <!-- <Col span="12" v-if="this.flag === 2 || this.flag === 4">
              <FormItem
                :label="$t('baoguanzuzhi')"
                style="width: 100%; height: 60px"
              >
                <Button @click="selectOrg(4)" style="width: 100%"
                  >选择组织</Button
                >
                <Input
                  type="textarea"
                  v-model="addformbase.manageOrganizeName"
                  readonly
                />
              </FormItem>
            </Col> -->
...
import { unitOfMeasure } from '@/api/unitOfMeasure';
import { storage } from '@/api/storageLocation';
export default {
  name: 'addModal',
  components: {
    addemp,
  ...
  methods: {
    ...
    handsave () {
      ...
      this.addformbase.assetDetailId = this.addformbase.assetId;
      switch (this.flag) {
        case 1:
          assetDetail.updateLose(this.addformbase).then((res) => {
            console.log('res=============', res);
          ...
          break;
        case 2:
          assetDetail.updatedRepair(this.addformbase).then((res) => {
            console.log('res=============', res);
            this.modal_loading = false;
            this.$emit('updateStat', false);
          });
          break;
        case 3:
          assetDetail.updatedReturn(this.addformbase).then((res) => {
            console.log('res=============', res);
            this.modal_loading = false;
            this.$emit('updateStat', false);
          });
          break;
        case 4:
          assetDetail.updateBorrow(this.addformbase).then((res) => {
            console.log('res=============', res);
            this.modal_loading = false;
            this.$emit('updateStat', false);
          });
          break;
        case 5:
          assetDetail.updatedScrap(this.addformbase).then((res) => {
            console.log('res=============', res);
            this.modal_loading = false;
            this.$emit('updateStat', false);
          });
    ...
    async getwelfareList () {
      ...
      try {
        ...
        this.addformbase.assetId = data.id;
      } catch (e) {
        // TODO zhuoda sentry
        console.error(e);
      }
