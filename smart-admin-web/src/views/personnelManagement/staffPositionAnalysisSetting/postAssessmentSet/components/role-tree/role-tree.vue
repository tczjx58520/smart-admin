  ...
  <div id="tree">
    ...
    <CheckboxGroup v-model="checkedData">
      <div class="checked-box">
        <ul>
          <!--li 一级权限模块 start-->
          <li :key="module.id" v-for="(module, moduleIndex) in tree">
            <div class="level-one">
              <!-- <Checkbox
                :label="module.id"
                @click.prevent.native="handleall(tree, moduleIndex)"
              >{{module.name}}</Checkbox> -->
              <Icon v-if="!module.value" type="md-add" @click.prevent.native="handleall(tree, moduleIndex)">
              </Icon>
              <Icon v-else type="md-trash" @click.prevent.native="handleall(tree, moduleIndex)">
              </Icon>
            ...
            >
              ...
              <Icon v-if="!childrenModule.value" type="md-add" @click.prevent.native="handleCheckAll(module.authorityVos,childrenModuleIndex,module.id,module)">
              </Icon>
              <Icon v-else type="md-trash" @click.prevent.native="handleCheckAll(module.authorityVos,childrenModuleIndex,module.id,module)">
              </Icon>
                ...
                <template v-for="(pages,pagesIndex) in childrenModule.authorityDetails">
                  ...
                  >
                    <Checkbox
                      :key="pages.key"
                      :label="pages.key"
                      @click.prevent.native="selectCheckbox(childrenModule.authorityDetails,pagesIndex,module.authorityVos)"
                    ...
                    <div :key="pagesIndex" class="Level-four" v-if="pages.children.length > 0">
                      <template v-for="(page, pageIndex) in pages.children">
                        <Checkbox
                          :key="page.key"
                          :label="page.key"
                          @click.prevent.native="selectCheckbox(pages.children, pageIndex,childrenModule.children,module.authorityVos)"
                        >{{page.name}}</Checkbox>
                      </template>
                    </div>
                  </div>
                  <Checkbox
                    ...
                    @click.prevent.native="selectCheckbox2(childrenModule.authorityDetails,pagesIndex,childrenModule.id,module.id,childrenModule,module)"
                    v-else
                  >{{pages.name}}</Checkbox>
                </template>
              </div>
...
export default {
  ...
  watch: {
    ...
    selected () {
      ...
      if (this.selected.authority) {
        this.checkedData = this.selected.authority.split(',');
      }
    }
  },
  filters: {},
