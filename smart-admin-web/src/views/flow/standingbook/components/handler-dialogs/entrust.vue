    ...
    <div>
      <Card dis-hover>
        ...
        <Form
          ...
          :rules="ruleValidate"
        >
          <FormItem :label="$t('wtr')" prop="entrustedPersonName">
            <Input
              v-model="addformbase.entrustedPersonName"
              @click.native="showemp_exa"
            ></Input>
          </FormItem>
          <FormItem :label="$t('wtyy')">
            <Input
              v-model="addformbase.entrustReason"
              type="textarea"
              style="width: 100%"
            ></Input>
          </FormItem>
...
export default {
  ...
  methods: {
    ...
    handsave () {
      ...
      const data = {
        entrustPersonId: this.actionInfo[0].handleRecordVos[0].flowRecordId,
        entrustedPersonId: this.addformbase.entrustedPersonIds,
        flowNumber: this.actionInfo[0].flowNumber,
      ...
      entrust.addentrust(data).then((res) => {
        if (res.ret === 200) {
