    ...
    <div>
      <Card dis-hover>
        ...
        <Form
          ...
          :rules="ruleValidate"
        >
          <FormItem :label="$t('pyyj')">
            <Input
              v-model="addformbase.reviewContent"
              type="textarea"
              style="width: 100%"
            ></Input>
          </FormItem>
...
export default {
  ...
  methods: {
    ...
    handsave () {
      ...
      const data = {
        distributionRecordId: this.distributionRecordId,
        reviewContent: this.addformbase.reviewContent
      };
      distribute.distributionHandle(data).then((res) => {
        if (res.ret === 200) {
