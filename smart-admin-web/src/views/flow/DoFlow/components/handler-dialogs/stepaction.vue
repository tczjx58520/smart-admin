    ...
    <div>
      <Card dis-hover>
        ...
        <Form
          ...
          :rules="ruleValidate"
        >
          <FormItem :label="FIlter(stat)">
            <Input
              v-model="addformbase.handleResult"
              type="textarea"
              style="width: 100%"
            ></Input>
          </FormItem>
...
export default {
  ...
  methods: {
    ...
    handsave () {
      ...
      const data = {
        id: this.actionInfo[0].handleRecordVos[0].id,
        actualPersonId: this.$store.state.user.userLoginInfo.userId,
        stat: this.stat,
        handleResult: this.addformbase.handleResult
      };
      unDoFlowApi.updateRecordDetail(data).then((res) => {
        if (res.ret === 200) {
