...
export default {
  ...
  data () {
    const validatePass = (rule, value, callback) => {
      console.log('测试时间=========================', this.addformbase.beginTimeStr === '');
    ...
    return {
      ...
      addformbase: {
        entrustPersonName: this.$store.state.user.userLoginInfo.nickName,
        entrustPersonId: this.$store.state.user.userLoginInfo.userId,
        beginTimeStr: utils.getDateStr(0, 'YMD'),
        entrustReason: '',
        beginTime: utils.getDateStr(0, 'YMD')
      },
      formList: [],
      cityList: [
        { value: 1, label: '重要' },
  ...
  methods: {
    ...
    getProcessList () {
      ...
      FlowCategoryApi.getEmpStart(listquery).then(response => {
    ...
    handsave () {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          entrust.addentrustedResult(data).then((res) => {
            if (res.ret === 200) {
