<template>
  ...
  >
    ...
    <!-- 新建员工弹窗 -->
    <!-- 新建结束============= -->
    <addContractModal :modalstat="visiable" @updateStat="updateStat"></addContractModal>
    <addemp
...
export default {
  ...
  methods: {
    ...
    updateStat (stat, row) {
      ...
      this.addformbase.contactId = row.id;
      this.addformbase.contactName = row.name;
      this.addformbase.organizationName = row.organizationName;
    },
    ...
    handsave () {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          publicEventsList.addstorage(this.addformbase).then(res => {
            if (res.ret === 200) {
              this.$emit('updateStat', false);
              this.addformbase = Object.assign({}, defaultForm);
