...
import addContractModal from '../addcontract/modal';
...
const defaultForm = {
  handPersonName: store.state.user.userLoginInfo.nickName,
  handPersonId: store.state.user.userLoginInfo.userId
};
export default {
  name: 'addModal',
  components: {
  ...
  methods: {
    ...
    updateStat_emp (stat, row) {
      ...
      this.addformbase.handPersonName = row.personName;
      this.addformbase.handPersonId = row.id;
    },
    handleSelectContact () {
      this.visiable = true;
    },
    ...
    updateStat (stat, row) {
      ...
      this.addformbase.customerId = row.id;
      this.addformbase.customerName = row.name;
      this.addformbase.telephone = row.telephone;
    },
    ...
    handsave () {
      this.addformbase.status = 0;
      this.$refs['form'].validate((valid) => {
        if (valid) {
          customerComplaintsList.addstorage(this.addformbase).then((res) => {
            if (res.ret === 200) {
              this.$emit('updateStat', false);
              this.addformbase = Object.assign({}, defaultForm);
