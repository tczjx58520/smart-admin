      ...
      <Card dis-hover>
        ...
        >
          ...
          <FormItem :label="$t('kqgl.jingdu')" prop="longitude">
            <Input
              style="width: 34%"
              @click.native="selectLong"
              v-model="fromBaseData.longitude"
            />
...
export default {
  ...
  watch: {
    ...
    editData() {
      ...
      let firstTextDataEmp1 = this.editData.attendaceAddressForEmpolyee.filter(
        (item) => {
          return item.status === 0;
        }
      );
      console.log(firstTextDataEmp1);
      let firstTextDataOrg1 = this.editData.attendanceAddressForOrganization.filter(
        (item) => {
          return item.status === 0;
        }
      );
      ...
      let firstTextDataEmp2 = this.editData.attendaceAddressForEmpolyee.filter(
        (item) => {
          return item.status === 1;
        }
      );
      console.log(firstTextDataEmp1);
      let firstTextDataOrg2 = this.editData.attendanceAddressForOrganization.filter(
        (item) => {
          return item.status === 1;
        }
      );
