    ...
    <div>
        <TabPane :label="$t('kqgl.bc')">
            <div style="margin-bottom:20px;">
              <Button style="margin-right:15px;" @click="resetFirstTable" icon="md-refresh" type="default">{{ $t('Reflash') }}</Button>
              <Button style="margin-right:15px;" @click="newFirstForm" icon="md-add" type="warning">{{ $t('Create') }}</Button>
              <Button style="margin-right:15px;" @click="clearFirstTable" icon="md-close" type="error">{{ $t('Delete') }}</Button>
            </div>
            <Table :columns="firstColumns" :data="firstData" :loading="firstLoading" @on-selection-change="selectFirst"></Table>
            <Page :current="firstTable.pageNum" :page-size="firstTable.pageSize" :page-size-opts="[10, 20, 30, 50, 100]"
            :total="fistTotal" @on-change="firstChangePage" @on-page-size-change="firstChangePageSize" show-elevator show-sizer
            show-total style="margin:24px 0;text-align:right;"></Page>
        </TabPane>
        <firstForm :modalstat.sync="modalstat" :modalState.sync="modalState" :editData="editData" @restList="restList"/>
    </div>
</template>
