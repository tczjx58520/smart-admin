<template>
        ...
        <div slot="header" style="text-align:left;color:#fff;">
            <span>{{ $t('kqgl.qingjiashengqindan') }}</span>
        </div>
        <div>
            <Card dis-hover>
                ...
                <Form ref="form" :model="fromBaseData" label-position="right" :label-width="100" :rules="ruleValidate">
                  <FormItem :label="$t('kqgl.liuchengbianhao')">
                    ...
                    <div class="nomalDiv">
                        ...
                        <InputNumber :max="100" :min="0" v-model="fromBaseData.useAnnual"  style="width: 20%"></InputNumber>
                        <div class="zhi">{{$t('kqgl.tian')}}</div>
                        <div class="zhi">{{$t('kqgl.shiyongtiaoxiu')}}</div>
                        <InputNumber :max="100" :min="0" v-model="fromBaseData.useExchangeDay"  style="width: 20%"></InputNumber>
                        <div class="zhi">{{$t('kqgl.tian')}}</div>
                    </div>
                    <div class="nomalDiv">
...
export default {
  ...
  methods: {
    ...
    getTimes (val) {
      ...
      let days = this.datedifference(val[1], val[0]) + 1;
      this.fromBaseData.totalTime = days;
    },
    chooseOrg () {
      this.orgStat = true;
    },
    ...
    handsave () {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          attendance.addApplyLeave(this.fromBaseData).then(res => {
            if (res.ret === 200) {
