...
export default {
  ...
  watch: {
    async modalstat() {
      ...
      let salary = await Promise.all(
        this.editinfo.map(async (item) => {
          let data = {};
          data.pageNum = 1;
          data.pageSize = 10;
          data.empId = item.id;
      ...
      this.querysalary = salary.map((item) => {
        return item.data.content.list;
      });
      let postcolumns = await Promise.all(
      ...
      for (let i = 0; i < this.baseinfo.length; i++) {
        ...
        this.baseinfo[i].accumulationFundId =
          processcolumns[i].accumulationFundId;
        if (
          this.baseinfo[i].salaryinfo !== null &&
          this.baseinfo[i].salaryinfo !== undefined
        ) {
          this.baseinfo[i].yearAndMonth = this.baseinfo[
            i
          ].salaryinfo.yearAndMonth;
          this.baseinfo[i].grantDate = this.baseinfo[i].salaryinfo.grantDate;
          this.baseinfo[i].confirmStat = this.baseinfo[
            i
          ].salaryinfo.confirmStat;
        }
      }
      // 动态生成报表项
      for (let i = 0; i < this.baseinfo.length; i++) {
          ...
          if (key === "salaryOptionVos") {
            for (const j in this.baseinfo[i].account[key]) {
              ...
              if (
                ...
                this.baseinfo[i][
                  this.baseinfo[i].account[key][j].name
              ...
              if (
                this.baseinfo[i].account[key][j].type === 4 ||
                this.baseinfo[i].account[key][j].type === 1
              ) {
                this.baseinfo[i]["formula"].push(
                  this.baseinfo[i].account[key][j].calculationFormulaVo
                );
              }
            }
          }
  ...
  methods: {
      ...
      let processcolumns = mycolumns.map((item) => {
        return {
          ...
          render: (h, params) => {
            ...
            if (
              params.row[item.name] !== undefined &&
              (item.name === "应发工资" ||
                item.name === "个人所得税" ||
                item.name === "实发工资" ||
                item.name === "养老保险" || 
            ...
            } else if (params.row[item.name] !== undefined) {
              return h("Input", {
                props: {
                ...
                on: {
                  input: (val) => {
                    ...
                    console.log(
                      "val========>触发==========333333333333",
                      this.data[params.index].formula
                    );
    ...
    async handsave() {
      ...
      for (let i = 0; i < this.baseinfo.length; i++) {
        ...
        for (const key in this.baseinfo[i]) {
          count++;
          if (count > 14) {
            const index = this.baseinfo[i].salaryOptionJson.findIndex( item=> {
              return item.salaryOptionName === key
            })
            this.baseinfo[i].salaryOptionJson[index].optionMoney = this.baseinfo[i][key]
          }
          if (key === "实发工资") {
          ...
          console.log(this.baseinfo[i].salaryOptionJson);
        }
      }
      for (let j = 0; j < this.baseinfo.length; j++) {
        if (this.baseinfo[j].isEnter === 0) {
          ...
          this.baseinfo[j].salaryOptionJson = JSON.stringify(
            this.baseinfo[j].salaryOptionJson
          );
    ...
    calc(params, mykey) {
      ...
      if (this.data[params.index].formula) {
        for (const i in this.data[params.index].formula) {
          if (!this.data[params.index].formula[i] || this.data[params.index].formula[i].salaryOptionName === '个人所得税') {
            continue;
          }
          console.log('mykey=====', this.data[params.index].formula[i].salaryOptionName);
          let process = [];
          myformula = this.data[params.index].formula[i].formula.replace(
            /#(.*?)#/g,
            function (word, $1) {
              process.push($1);
              return $1;
          ...
          for (const key in this.data[params.index]) {
            if (process.findIndex((val) => val === key) > -1) {
              console.log(
                "计算属性===========================>",
                process.findIndex((val) => val === key),
                key
              );
              process.splice(
                process.findIndex((val) => val === key),
                1,
                `(${Number(this.data[params.index][key])})`
              );
            }
          }
          console.log("process.join===================>", process.join(""));
          // eslint-disable-next-line
          let res = eval(process.join("")).toFixed(
            this.data[params.index].formula[i].pointNum
          );
          console.log("res======================>", res);
          // this.data[params.index][this.data[params.index].formula[i]] = res;
          this.data[params.index][
            this.data[params.index].formula[i].salaryOptionName
          ] = res;
          this.$set(
            params.row,
            this.data[params.index].formula[i].salaryOptionName,
          ...
          if (
              this.data[params.index].formula[i].salaryOptionName === "应发工资"
            ) {
              this.calcSoloTax(params, Number(res));
              continue;
            }
    ...
    // 个人所得税
    calcSoloTax(params,money) {
      console.log('money==========================', money);
      let should = Number(money) - 5000;
      let tax = 0.0;
      if (should > 80000) {
        let tax1 = (should - 80000) * 0.45;
        tax = tax + tax1;
        let tax2 = 25000 * 0.35;
        tax = tax + tax2;
        let tax3 = 20000 * 0.3;
        ...
        let tax7 = 3000 * 0.03;
        tax = tax + tax7;
      } else if (should < 80000 && should >= 55000) {
        let tax2 = (should - 55000) * 0.35;
        tax = tax + tax2;
        let tax3 = 20000 * 0.3;
        tax = tax + tax3;
        let tax4 = 10000 * 0.25;
        ...
        let tax7 = 3000 * 0.03;
        tax = tax + tax7;
      } else if (should < 55000 && should >= 35000) {
        let tax3 = (should - 35000) * 0.3;
        tax = tax + tax3;
        let tax4 = 10000 * 0.25;
        tax = tax + tax4;
        let tax5 = 13000 * 0.2;
        ...
        let tax7 = 3000 * 0.03;
        tax = tax + tax7;
      } else if (should < 35000 && should >= 25000) {
        let tax4 = (should - 25000) * 0.25;
        tax = tax + tax4;
        let tax5 = 13000 * 0.2;
        tax = tax + tax5;
        let tax6 = 9000 * 0.1;
        ...
        let tax7 = 3000 * 0.03;
        tax = tax + tax7;
      } else if (should < 25000 && should >= 12000) {
        let tax5 = (should - 12000) * 0.2;
        tax = tax + tax5;
        let tax6 = 9000 * 0.1;
        tax = tax + tax6;
        let tax7 = 3000 * 0.03;
        tax = tax + tax7;
      } else if (should < 12000 && should >= 3000) {
        let tax6 = (should - 3000) * 0.1;
        tax = tax + tax6;
        let tax7 = 3000 * 0.03;
        tax = tax + tax7;
      } else if (should < 3000 && should >= 0) {
        let tax7 = should * 0.03;
        tax = tax + tax7;
      }
      console.log('个人所得税=========', tax);
      this.data[params.index]['个人所得税'] = tax;
      this.$set(
        params.row,
        '个人所得税',
        tax
